...
import 'package:geolocator/geolocator.dart';
    ...
    class _LiveRideScreenState extends State<LiveRideScreen> {
      ...
      void _showOtpDialog(String orderId) {
        ...
        // ✅ DIALOG KHULNE SE PEHLE capture karo — socket ya liveApi se
        final savedOtp = rideVm.activeRideData?['otp']?.toString().isNotEmpty == true
            ? rideVm.activeRideData!['otp'].toString()
            : liveRideVm.liveOrderModel?.data?.otp?.toString() ?? '';

        print("🔐 OTP Source — Socket: ${rideVm.activeRideData?['otp']} | LiveAPI: ${liveRideVm.liveOrderModel?.data?.otp}");
        print("🔐 Final savedOtp: $savedOtp");
                          ...
                          child: ElevatedButton(
                            style: ElevatedButton.styleFrom(
                              ...
                              onPressed: () async {
                                ...
                                print("🔐 Saved OTP: $savedOtp | Entered: $enteredOtp"); // ✅ closured savedOtp use hoga

                                if (savedOtp == enteredOtp) {
                                  // ✅ Optimistic update PEHLE
                                  setState(() {
                                    _localRideStatus = 4;
                                    isOtpVerified = true;
                                  });
                                  ...
                                  updateRideStatusVm.updateRideApi(context, orderId,enteredOtp, "","","4").then((_) {
                                    ...
                                    Utils.showErrorMessage(context, "Failed: $e");
                                  });

                                } else {
                                  Utils.showErrorMessage(context, loc.invalid_otp_try_again);
                                }
                            ...
                            child: InkWell(
                              onTap: () async {
                                ...
                                try {
                                  ...
                                  // ✅ Error aaye toh rollback karo
                                  setState(() => _localRideStatus = currentStatus);
                                  Utils.showErrorMessage(
                                      context, "${loc.failed_update_ride_status} $e");
                                }
                              },
